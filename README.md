...
## Column-by-Column Cleaning Policy — v3

The manual Smart Preprocessing page is intentionally column-by-column. There is no bulk cleaning-strategy comparison and no encode-all control. Cross Validation is used for model comparison/selection, not for automatically deciding how to clean each individual column.

KNN imputation can use other numeric features to identify neighbours, but only the selected column is modified.


## Unsupervised Evaluation & Inference (v4)

Unsupervised Learning is now connected to the existing **Evaluation** and **Inference** pages instead of ending after training.

- **K-Means:** Silhouette, Davies-Bouldin, Calinski-Harabasz, inertia, cluster sizes/profiles, native prediction for new rows.
- **Hierarchical:** internal clustering metrics, sizes/profiles, and nearest-centroid assignment for future rows (clearly labeled because AgglomerativeClustering has no native `predict`).
- **DBSCAN:** cluster/noise metrics plus new-row assignment to the nearest fitted core cluster when within `eps`; otherwise `-1` noise.
- **PCA:** explained-variance evaluation and native `transform` for single, 10-row, or batch future data.
- **t-SNE:** trustworthiness evaluation; no fake future-row prediction is provided because sklearn t-SNE has no out-of-sample transform.
- **Apriori:** rule count/confidence/lift evaluation and matching-rule recommendations for a new transaction.

The app keeps the same page count: no new sidebar pages were added.
